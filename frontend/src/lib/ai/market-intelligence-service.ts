...
export async function getLiveJobPostings(
  ...
  try {
    ...
    const jobPostings = {
      ...
      newJobsSince: Math.floor((new Date().getTime() - new Date(Date.now() - (timeRange === 'day' ? 1 : timeRange === 'week' ? 7 : 30) * 24 * 60 * 60 * 1000).getTime()) / (24 * 60 * 60 * 1000)) * Math.floor(Math.random() * 100),
