...
export default function ScreeningQuestionsPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.warn('AI API unavailable, using high-quality demo fallback screening questions:', error);
      setIsGenerating(false);
      const demoQuestions = [
        {
          text: `Can you walk us through a recent project where you utilized ${filters.experience || 'modern frameworks'} to solve a complex performance or architectural challenge?`,
          category: 'technical',
          difficulty: filters.skillLevel,
          type: 'screening'
        },
        {
          text: `How do you structure code quality, testing, and error handling for critical full-stack workflows?`,
          category: 'technical',
          difficulty: filters.skillLevel,
          type: 'screening'
        },
        {
          text: `Tell us about a time when you had to push back on unrealistic product deadlines or scope changes while keeping stakeholders satisfied.`,
          category: 'behavioral',
          difficulty: filters.skillLevel,
          type: 'screening'
        },
        {
          text: `Describe a situation where you mentored or collaborated with junior team members during high-pressure delivery windows.`,
          category: 'behavioral',
          difficulty: filters.skillLevel,
          type: 'screening'
        },
        {
          text: `Imagine a production outage occurs during peak traffic due to a database lock. What are the immediate steps you take to diagnose and resolve it?`,
          category: 'scenario',
          difficulty: filters.skillLevel,
          type: 'screening'
        },
        {
          text: `If tasked with migrating a legacy service to a microservice architecture without downtime, what deployment and canary strategies would you employ?`,
          category: 'scenario',
          difficulty: filters.skillLevel,
          type: 'screening'
        }
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      {(generatedQuestions.length > 0 || customQuestions.length > 0) && (
        <div className="space-y-4">
          ...
          {Object.entries(groupedQuestions).map(([category, questionsList]) => {
            const questions = (questionsList as any[]) || [];
            return (
