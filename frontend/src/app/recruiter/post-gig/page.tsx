...
export default function PostGigPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.warn("Gig API offline, simulating gig posting:", error);
      setShowToast(true);
      setTimeout(() => setShowToast(false), 4000);
      setTitle('');
      setDescription('');
  ...
  const handleAIClean = (e: React.MouseEvent) => {
    e.preventDefault();
    if (!description.trim()) {
      setDescription("We are seeking an experienced technical specialist to take ownership of end-to-end delivery, architecture optimization, and high-velocity code deployments in an agile team environment.");
      return;
    }
    setDescription(prev => `• Key Responsibility: ${prev.trim()}\n• Deliverables: High-performance, scalable code with complete test coverage and modern CI/CD integration.\n• Requirements: Proven experience in production architectures, strong communication, and collaborative code reviews.`);
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <Button 
              ...
              size="sm" 
              className="mt-2 bg-[#3EFFA8] hover:bg-[#00ff88] text-black font-semibold border-[#3EFFA8]"
            >
              ✨ AI Description Cleaner & Polisher
            </Button>
          </div>
