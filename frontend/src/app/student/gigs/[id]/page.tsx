...
import { studentGigsList } from '@/lib/studentMockData'
  ...
  const {
    ...
    queryFn: async () => {
      try {
        ...
        const source = gigsData.length > 0 ? gigsData : studentGigsList
        return source.find((g) => g.gigId === gigId)
      } catch (error) {
        console.warn('Using demo gig detail:', error)
        return studentGigsList.find((g) => g.gigId === gigId)
      }
    },
    enabled: !!gigId,
  })
