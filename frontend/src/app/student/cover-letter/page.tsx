...
import { FileText, Sparkles, Download, Eye, Edit3, CheckCircle, Zap, Upload, Target, Mail, Loader2, AlertCircle } from 'lucide-react'
import { GlassCard } from '@/components/cards/GlassCard'
import { generateCoverLetter, generateResumeFromProfile, optimizeResumeATS, ResumeData } from '@/lib/ai/openai-service'
import { getMockCoverLetter, getMockATSResult } from '@/lib/ai/mock-ai-service'
...
export default function AICoverLetterGenerator() {
  ...
  const generateCoverLetterAI = async () => {
    ...
    try {
      ...
      const toneMap: Record<string, 'professional' | 'enthusiastic' | 'confident'> = {
      ...
      try {
        ...
        generatedContent = await generateCoverLetter(
          resumeData,
        ...
        generatedContent = getMockCoverLetter(companyName, jobPosition)
        setIsDemoMode(true)
      }
      ...
      let analysis
      try {
        analysis = await optimizeResumeATS(generatedContent, jobDescription)
      } catch {
        analysis = getMockATSResult(generatedContent)
        setIsDemoMode(true)
      }
      ...
      {isDemoMode && (
        <div className="flex items-center gap-3 p-4 bg-amber-500/10 border border-amber-400/30 rounded-xl">
          ...
          <div className="flex-1">
            ...
            <span className="text-amber-200/70 text-sm ml-2">— AI API key not configured. Showing realistic sample output. Add your OpenAI key to <code className="bg-amber-900/30 px-1 rounded">.env.local</code> for live AI.</span>
          </div>
        </div>
      )}
      {/* Header */}
