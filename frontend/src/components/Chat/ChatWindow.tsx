...
export default function ChatWindow({ conversationWithId, currentUserId, partnerName }: ChatWindowProps) {
  ...
  async function loadMessages() {
    try {
      const res = await fetch(`/api/recruiter/chat/get-messages?user1=${currentUserId}&user2=${conversationWithId}`)
      const json = await res.json()
      if (json.success) {
        setMessages(json.messages)
        // Mark messages as read
        await fetch('/api/recruiter/chat/mark-read', {
          method: 'POST',
  ...
  async function sendMessage() {
    ...
    try {
      const res = await fetch('/api/recruiter/chat/send-message', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
