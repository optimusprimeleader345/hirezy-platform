...
import { calculateLocalJobMatch } from '@/lib/student-mock-data'
  ...
  try {
    ...
    if (pythonMatch) {
      ...
      try {
        ...
        console.warn('Using local job match fallback:', aiError);
        matchResult = calculateLocalJobMatch(
          studentData.skills,
          jobData.skills,
          title
        );
    ...
    try {
      ...
      const studentSkills = body.studentSkills || body.studentProfile?.skills || []
      const jobSkills = Array.isArray(body.skills) ? body.skills : []
      const fallback = calculateLocalJobMatch(studentSkills, jobSkills, body.title || 'Job')
      ...
      return NextResponse.json({
        ...
        _meta: {
          processedAt: new Date().toISOString(),
          algorithmVersion: '1.0.0-local-fallback',
          dataSource: 'local_skill_matching'
        }
      })
    } catch {
      return NextResponse.json(
